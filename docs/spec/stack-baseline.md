...
|---|---|---|
| **Project architecture** — ESM, strict TS, `tsc`-only, transport-agnostic `buildServer()`, one file per tool, native `fetch` | verbatim | **+ protobuf dependency `gtfs-realtime-bindings`** (GTFS-RT is protobuf-only — `?format=json` ignored; ticket 001; decoder chosen in [ticket 008](./realtime-integration.md)). **+ GTFS-ingestion dependency `@libsql/client`** (Turso; ticket 006 — node-gtfs rejected). go-planner's "GTFS-RT as JSON, no protobuf dep" does **not** carry over. |
