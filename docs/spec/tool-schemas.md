...
**6. `get_arrivals`** — `{ stop_id, route_id?, limit? }` → `{ stop, arrivals: Arrival[], realtime_available: boolean, truncated }`. **Unified** (ticket 005): live predictions for bus/streetcar; for subway transparently returns scheduled times. `Arrival { route_id, route_short_name, headsign, direction_id, time, realtime: boolean, source: "predicted" | "scheduled", delay_seconds? }`. For a station, grouped by direction. Field shape is **finalized in ticket 008**; kept compatible here.
